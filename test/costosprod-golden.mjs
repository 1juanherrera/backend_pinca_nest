...
let sid;
try{
  await q("DELETE FROM costos_snapshot WHERE item_general_id=999001");
  sid=(await q("INSERT INTO costos_snapshot (item_general_id,fecha,estado,costo_mp_por_unidad,costo_empaque_mod,costo_total,precio_venta_calc,mps_total,mps_cubiertas) VALUES (999001,'2026-01-01','completo',12.5,3.25,15.75,20.0,4,3)")).insertId;
  await q("INSERT INTO costos_snapshot (item_general_id,fecha,estado,costo_mp_por_unidad,costo_empaque_mod,costo_total,precio_venta_calc,mps_total,mps_cubiertas) VALUES (999001,'2026-02-01','incompleto',NULL,1.0,NULL,NULL,4,2)");
  console.log('');
  check('historia con snapshots', await api(CI4,'/api/costos-produccion/999001/historia'), await api(NEST,'/api/costos-produccion/999001/historia'));
  check('historia sin snapshots (vacío)', await api(CI4,'/api/costos-produccion/999999/historia'), await api(NEST,'/api/costos-produccion/999999/historia'));
  console.log(`\n═══ RESULTADO: ${pass} OK, ${fail} FALLOS ═══\n`);
}finally{ await q("DELETE FROM costos_snapshot WHERE item_general_id=999001").catch(()=>{}); await db.end(); }
